...
/**
 * Client HTTP unique du projet, appelé depuis les composants client contre
 * le proxy `/api/*` (jamais directement contre le backend) — voir
 * darmeuble-kit/docs/frontend/contrat-api.md.
 */
...
async function executer<T>(chemin: string, options: OptionsRequete = {}) {
...
export const api = {
   ...
   * fournie par le backend dès le départ (`PaginationQueryDto`).
   */
  getPage: async <T>(
    chemin: string,
    params: OptionsRequete["params"],
