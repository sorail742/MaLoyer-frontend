 ...
 * darmeuble-kit/docs/backend/adr/0004-*.md, docs/frontend/adr/0002-*.md),
 * et ne renvoie jamais `accessToken` en clair dans le corps JSON transmis
 * au navigateur — le cookie suffit, le code applicatif frontend n'a jamais
 * besoin de le lire lui-même.
 *
 * Utilisé par les Route Handlers dédiés aux routes d'authentification
 * (`app/api/auth/login`, `app/api/auth/otp/verify`), **pas** par le proxy
 * générique `[...chemin]`, qui ne fait que relayer sans inspecter les
 * corps de réponse.
 */
