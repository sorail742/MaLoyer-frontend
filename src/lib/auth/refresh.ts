 ...
 * jamais exposé comme route publique (voir
 * darmeuble-kit/docs/backend/adr/0004-*.md,
 * docs/frontend/adr/0002-*.md). Le navigateur n'appelle jamais
 * `/api/auth/refresh` lui-même.
 *
 * Un seul appel en vol par processus serveur — le backend ne traite pas
 * deux appels concurrents sur le même refresh token avec indulgence
 * (réutilisation détectée = révocation de session entière, ADR-0004),
 * c'est ce verrou qui les sérialise côté frontend.
 */
