 ...
 * darmeuble-kit/docs/backend/adr/0004-*.md et docs/frontend/adr/0002-*.md.
 *
 * **Ne gère jamais les routes d'authentification qui émettent ou révoquent
 * un jeton** (`/api/auth/login`, `/api/auth/otp/verify`,
 * `/api/auth/logout`) — elles ont leurs propres Route Handlers dédiés
 * (`app/api/auth/`), parce que leur réponse a une forme particulière à
 * traiter (voir `lib/auth/process-auth-response.ts`). Ce fichier ne fait
 * que relayer.
 */
...
async function relayer(
  ...
  // (ci-dessous) a besoin de le renvoyer une seconde fois — compromis
  // assumé sur les fichiers volumineux (aucun endpoint Phase 1 n'en
  // envoie).
  ...
  if (reponse.status === 401 && relatif !== "auth/refresh") {
    ...
    // voir darmeuble-kit/docs/backend/adr/0004-*.md) : la session est
    // terminée.
